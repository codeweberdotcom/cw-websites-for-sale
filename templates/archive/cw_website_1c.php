<?php
/**
 * Archive: Websites For Sale — Template 1c (Price Pill + Dark Footer)
 */
...
?>
<style>
.cw-wfs-1c-frame   { position:relative; height:176px; overflow:hidden; }
.cw-wfs-1c-frame img { width:100%; height:100%; object-fit:cover; object-position:top center; display:block; }
.cw-wfs-1c-pill    { position:absolute; top:12px; right:12px; display:inline-flex; align-items:center; height:30px; padding:0 14px; border-radius:999px; font-size:14px; font-weight:800; background:#fff; color:#0f172a; box-shadow:0 3px 10px rgba(15,23,42,.2); z-index:2; }
...
.cw-wfs-1c-frame:hover .cw-wfs-ov { opacity:1; }
.cw-wfs-1c-dot     { width:8px; height:8px; border-radius:50%; flex-shrink:0; }
.cw-wfs-1c-footer  { display:grid; grid-template-columns:1fr 1fr; gap:1px; background:#1e293b; margin-top:auto; }
.cw-wfs-1c-btn     { display:flex; align-items:center; justify-content:center; height:48px; background:#0f172a; color:#fff; font-weight:700; font-size:14px; text-decoration:none; }
.cw-wfs-1c-btn:hover { background:#1e293b; color:#fff; }
.cw-wfs-1c-btn-sec { color:#cbd5e1; }
.cw-wfs-1c-btn-sec:hover { background:#1e293b; color:#fff; }
</style>
		...
		<div class="col-md-6 col-xl-4">
			<div class="card shadow-sm border-0 rounded-4 overflow-hidden h-100">
				<div class="cw-wfs-1c-frame">
					...
					<?php endif; ?>
					<?php if ( $price ) : ?>
					<span class="cw-wfs-1c-pill"><?php echo esc_html( $price ); ?> ₽</span>
					<?php endif; ?>
				...
				<div class="card-body d-flex flex-column gap-3 p-4">
					<div class="d-flex align-items-center gap-2">
						<?php if ( $cat_name ) : ?>
						<span class="cw-wfs-1c-dot" style="background:<?php echo esc_attr( $cat_c['dot'] ); ?>;"></span>
				...
				<div class="cw-wfs-1c-footer">
					<a href="<?php echo esc_url( $permalink ); ?>" class="cw-wfs-1c-btn"><?php esc_html_e( 'Details', 'cw-websites-for-sale' ); ?></a>
					<?php if ( $website_url ) : ?>
					<a href="<?php echo esc_url( $website_url ); ?>" target="_blank" rel="noopener" class="cw-wfs-1c-btn cw-wfs-1c-btn-sec"><i class="uil uil-play-circle me-2"></i><?php esc_html_e( 'Preview', 'cw-websites-for-sale' ); ?></a>
					<?php else : ?>
					<span></span>
					<?php endif; ?>
				</div>
...
(function () {
	...
	function fetchFiltered() {
		...
		body.append( 'params', JSON.stringify({ post_type: 'cw_website', template: 'cw_websites_1c', filters: filters }) );
